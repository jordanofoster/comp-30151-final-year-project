...
- [COMP30151: Project Report - Jordan Foster (N0913799)](#comp30151-project-report---jordan-foster-n0913799)
  ...
  - [2 Context](#2-context)
    - [2.1 Current State](#21-current-state)
      - [2.1.1 Current uses of the DMS concept](#211-current-uses-of-the-dms-concept)
        - [2.1.1.1 Uses of the DMS concept as a defensive measure (*'fail-safe/secure' systems*)](#2111-uses-of-the-dms-concept-as-a-defensive-measure-fail-safesecure-systems)
          ...
          - [2.1.1.1.3 Stop mechanisms for machinery and tools alongside mobility and rehabilitation](#21113-stop-mechanisms-for-machinery-and-tools-alongside-mobility-and-rehabilitation)
    ...
    - [2.2 Limitations of current research and areas for further work](#22-limitations-of-current-research-and-areas-for-further-work)
      ...
      - [2.2.8 Lack of research into solutions identifying the need for 'covert' support of the victim in pre-separation stages of abuse](#228-lack-of-research-into-solutions-identifying-the-need-for-covert-support-of-the-victim-in-pre-separation-stages-of-abuse)
  ...
  - [4 Implementation and Investigation](#4-implementation-and-investigation)
    ...
    - [4.3 Implementation of usecase specific payload function](#43-implementation-of-usecase-specific-payload-function)
      ...
      - [4.3.2 Main payload function and general payload execution phases](#432-main-payload-function-and-general-payload-execution-phases)
    ...
    - [5.1 Test Definitions and Results](#51-test-definitions-and-results)
      ...
      - [5.1.3 Payload Function](#513-payload-function)
        ...
        - [5.1.3.2 `--alert`](#5132---alert)
          ...
          - [5.1.3.2.2 Using `--alert` to send signed, unencrypted message](#51322-using---alert-to-send-signed-unencrypted-message)
...
| `--reject-emotions` | `str:float` (in format `emotion:min_confidence`). | Space-separated list of forbidden emotions and the minimum averaged score to trigger upon, distinguished via `:`. Triggers when one or more identities hold a forbidden emotion with an averaged score *greater than or equal* to the minimum provided, calculated over a sliding window of the last <img src="https://latex.codecogs.com/gif.latex?x"/> frames the identity was recognized in (with <img src="https://latex.codecogs.com/gif.latex?x"/> provided by `--sliding-window-size`). | - At least one trigger condition must be provided. - Arguments must be in the format `emotion:score`. - `emotion` must be a valid `deepface` library emotion (happy, neutral, surprise, sad, angry, fear, disgust). - `min_score` must be within acceptable bounds (0-100). |
...
| `--sliding-window-size` | `int` | Defines the size of the 'sliding window' upon which FER averaging is performed for a given identity (across the last <img src="https://latex.codecogs.com/gif.latex?x"/> frames in which the identity was recognized). Smaller values result in forbidden emotions being checked more often, but increases false positive risk. Higher values are read less often but are more likely to be accurate. Default 10. | `--sliding-window-size` cannot be less than `--max-buffer-size`. |
...
| 2 | Confirm that the path provided by `PGP_PRIVKEY` is a valid PGP private key via `gpg --show-keys --with-fingerprint <img src="https://latex.codecogs.com/gif.latex?PGP_PRIVKEY`.%20|%20The%20command%20returns%20information%20indicating%20that%20this%20is%20a%20valid%20PGP%20private%20key%20capable%20of%20signing%20messages.%20|%20`gpg%20--show-keys%20--with-fingerprint%20dms.priv.asc`%20|%20`sec%20%20%20ed25519%202024-03-31%20[SC]%20[expires:%202026-03-31]%20%20%20%20%20%205F53%201337%20BAA9%2085A3%20F307%20%20938B%202C2D%209793%2062FC%20424F%20uid%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20Dead%20Man&#39;s%20Switch%20&lt;dms@example.com&gt;%20ssb%20%20%20cv25519%202024-03-31%20[E]%20[expires:%202026-03-31]`%20|%20PASS%20||%203%20|%20Start%20the%20DMS%20launcher%20program%20with%20valid%20arguments,%20ensuring%20that%20the%20`--alert`%20payload%20function%20argument%20flag%20has%20been%20provided%20and%20with%20observer%20function%20arguments%20that%20are%20likely%20to%20cause%20a%20trigger%20event%20in%20the%20current%20test%20environment.%20|%20The%20DMS%20launcher%20starts%20without%20issue.%20|%20`.\dms-launcher.py%20-p%20&quot;.\payload.py%20localhost%205455%20.\secrets\%20--heartbeat-grace-period%205%20--heartbeat-max-retries%205%20--heartbeat-timeout%205%20--handshake-timeout%205%20--alert%20--log-level%20INFO&quot;%20-o%20&quot;.\observer.py%20localhost%205455%20--heartbeat-grace-period%205%20--heartbeat-max-retries%205%20--heartbeat-timeout%205%20--handshake-timeout%205%20--reject-noframe%20--log-level%20INFO&quot;%20--timeout%205%20--retries%200%20--env%20.\Scripts\%20--log-level%20INFO`%20|%20As%20expected.%20|%20PASS%20||%204%20|%20Confirm%20that%20the%20observer%20and%20payload%20subprocesses%20have%20started.%20|%20The%20observer%20and%20payload%20process%20logs%20indicate%20that%20both%20processes%20have%20started%20successfully.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%205%20|%20Confirm%20that%20the%20observer%20and%20payload%20successfully%20handshake.%20|%20The%20observer%20and%20payload%20process%20logs%20indiate%20that%20a%20lifeline%20was%20established%20by%20both%20parties.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%206%20|%20Confirm%20that%20the%20DMS%20launcher%20has%20noted%20that%20both%20observer%20and%20payload%20are%20still%20alive.%20|%20The%20DMS%20launcher%20logs%20indicate%20that%20neither%20observer%20or%20payload%20processes%20ended%20within%20the%20timeout%20period.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%207%20|%20Confirm%20that%20heartbeats%20are%20being%20sent%20from%20the%20observer%20to%20the%20payload.%20|%20Heartbeat%20logs%20from%20a%20distinct%20process%20ID%20confirm%20the%20sending%20of%20heartbeat%20signals.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%208%20|%20Confirm%20that%20heatbeats%20are%20being%20received%20by%20the%20observer%20from%20the%20payload.%20|%20Heartbeat%20logs%20from%20the%20*same*%20process%20ID%20as%20step%207%20confirm%20the%20receipt%20of%20heartbeat%20signals.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%209%20|%20Confirm%20that%20heartbeats%20are%20being%20sent%20from%20the%20payload%20to%20the%20observer.%20|%20Heartbeat%20logs%20from%20a%20*different*%20process%20ID%20as%20in%20steps%207-8%20confirm%20the%20sending%20of%20heartbeat%20signals.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2010%20|%20Confirm%20that%20heartbeats%20are%20being%20received%20by%20the%20payload%20from%20the%20observer.%20|%20Heartbeat%20logs%20from%20the%20*same*%20process%20ID%20as%20in%20step%209%20confirm%20the%20receipt%20of%20heartbeat%20signals.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2011%20|%20Confirm%20that%20the%20observer%20process%20is%20actively%20running.%20|%20Observer%20function%20logs%20are%20present.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2012%20|%20Confirm%20a%20trigger%20event%20in%20the%20observer%20function.%20|%20The%20relevant%20observer%20function%20subprocedure%20logs%20indicate%20a%20trigger%20event,%20causing%20an%20exit.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2013%20|%20Confirm%20that%20the%20main%20observer%20function%20notices%20the%20trigger%20event%20and%20attempts%20a%20teardown.%20|%20The%20observer%20function%20logs%20indicate%20that%20a%20subprocedure%20has%20exited%20and%20evidence%20attempts%20to%20clean%20up%20and%20exit.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2014%20|%20Confirm%20that%20the%20observer%20*process*%20notes%20that%20the%20observer%20function%20has%20been%20exited%20and%20attempts%20to%20sever%20the%20lifeline.%20|%20The%20observer%20process%20logs%20indicate%20that%20the%20observer%20function%20has%20died%20and%20the%20lifeline%20has%20severed.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2015%20|%20Confirm%20that%20the%20payload%20*process*%20notes%20that%20the%20lifeline%20has%20been%20*explicitly*%20closed%20via%20the%20connection%20being%20reset.%20|%20The%20payload%20process%20logs%20state%20that%20the%20payload%20was%20triggered%20with%20no%20prior%20indication%20of%20lifeline%20failure.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2016%20|%20Confirm%20that%20the%20payload%20process&#39;%20trigger%20procedure%20begins.%20|%20The%20payload%20process%20logs%20indicate%20the%20main%20trigger%20function%20has%20been%20started.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2017%20|%20Confirm%20that%20the%20execution%20phase%20of%20the%20payload%20succeeds%20without%20error.%20|%20The%20payload%20function%20logs%20indicate%20that%20the%20execution%20phase%20succeeds%20without%20error.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2018%20|%20Confirm%20that%20the%20alert%20phase%20of%20the%20payload%20succeeds%20without%20error.%20|%20The%20payload%20function%20logs%20indicate%20that%20the%20alert%20phase%20succeeds%20without%20error.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2019%20|%20Confirm%20that%20the%20verification%20phase%20of%20the%20payload%20succeeds%20without%20error.%20|%20The%20payload%20function%20logs%20indicate%20that%20the%20verification%20phase%20succeeds%20without%20error.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2020%20|%20Confirm%20that%20the%20payload%20**function**%20appears%20to%20exit%20successfully.%20|%20The%20payload%20function%20logs%20indicate%20it%20has%20finished%20all%20phases%20successfully.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2021%20|%20Confirm%20that%20the%20payload%20**process**%20has%20exited%20cleanly.%20|%20The%20payload%20process%20logs%20indicate%20it%20has%20exited%20with%20`triggerFinishedException`.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2022%20|%20Confirm%20that%20the%20DMS%20alert%20e-mail%20has%20been%20received%20in%20the%20recipient&#39;s%20inbox.%20|%20The%20DMS%20can%20be%20seen%20to%20have%20been%20received%20in%20the%20recipient&#39;s%20inbox,%20with%20the%20subject%20&#39;DMS%20Triggered&#39;%20and%20a%20timestamp.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2023%20|%20Confirm%20that%20the%20contents%20of%20the%20DMS%20alert%20e-mail%20match%20those%20that%20were%20provided%20in%20the%20`MSG_CONTENTS`%20environment%20variable.%20|%20Contents%20of%20the%20DMS%20alert%20e-mail%20match%20those%20provided%20in%20`MSG_CONTENTS`%20as%20defined%20in%20step%201.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2024%20|%20Confirm%20that%20the%20DMS%20alert%20e-mail%20is%20recognized%20as%20being%20*signed*%20using%20the%20test%20client.%20|%20The%20test%20client%20used%20recognizes%20the%20alert%20e-mail%20is%20*signed*%20with%20the%20**same%20key%20ID**%20as%20noted%20in%20step%202.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2025%20|%20Confirm%20that%20the%20DMS%20alert%20e-mail%20is%20recognized%20as%20being%20*unencrypted*%20using%20the%20test%20client.%20|%20The%20test%20client%20used%20recognizes%20the%20alert%20e-mail%20is%20*unencrypted*.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2026%20|%20Confirm%20that%20the%20`.env`%20file%20has%20been%20deleted%20from%20the%20test%20directory.%20|%20The%20`.env`%20file%20no%20longer%20exists%20in%20the%20test%20directory.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2027%20|%20Confirm%20that%20the%20`.env`%20file%20has%20NOT%20been%20put%20in%20the%20recycling%20bin%20(or%20&#39;trash&#39;).%20|%20The%20`.env`%20file%20is%20not%20locatable%20within%20the%20recycling%20bin.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2028%20|%20Confirm%20that%20the%20`PGP_PRIVKEY`%20file%20has%20been%20deleted%20from%20the%20relevant%20directory.%20|%20The%20public%20key%20specified%20by%20`PGP_PRIVKEY`%20no%20longer%20exists.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%2029%20|%20Confirm%20that%20the%20`PGP_PRIVKEY`%20file%20has%20NOT%20been%20put%20in%20the%20recycling%20bin%20(or%20&#39;trash&#39;).%20|%20The%20public%20key%20specified%20by%20`PGP_PRIVKEY`%20is%20not%20locatable%20within%20the%20recycling%20bin.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20|Table%2041.%20Test%20script%20for%20sending%20signed,%20unencrypted%20`--alert`%20within%20usecase%20specific%20payload%20function.######%205.1.3.2.3%20Using%20`--alert`%20to%20unsigned,%20encrypted%20message-%20**Given**%20that%20I%20have%20an%20observer%20and%20payload%20function%20source%20file%20%20-%20**And**%20that%20the%20payload%20source%20file%20contains%20the%20payload%20function%20relevant%20to%20the%20usecase%20provided%20%20-%20**And**%20that%20all%20required%20arguments%20have%20been%20provided%20and%20are%20valid%20%20-%20**And**%20that%20the%20`--alert`%20argument%20flag%20has%20been%20provided%20to%20the%20payload%20function%20%20-%20**And**%20that%20a%20`.env`%20file%20with%20all%20required%20environment%20variables%20has%20been%20provided%20%20-%20**And**%20that%20the%20`.env`%20file%20contains%20the%20`PGP_PUBKEY`%20environment%20variable%20%20-%20**And**%20that%20the%20value%20of%20the%20`PGP_PUBKEY`%20environment%20variable%20represents%20a%20filepath%20to%20a%20valid%20public%20PGP%20key%20%20-%20**And**%20that%20the%20observer%20process&#39;%20trigger%20conditions%20have%20been%20setup%20such%20that%20they%20should%20be%20immediately%20met-%20**Then**%20when%20I%20start%20the%20DMS%20launcher%20%20-%20**Then**%20an%20observer%20and%20payload%20subprocess%20should%20be%20started%20%20%20%20-%20**And**%20heartbeats%20should%20be%20seen%20to%20be%20working%20%20%20%20-%20**And**%20the%20observer%20function%20should%20be%20noted%20to%20have%20setup%20successfully-%20**Then**%20when%20the%20observer%20function%20is%20triggered%20%20-%20**Then**%20the%20main%20observer%20function%20should%20be%20seen%20to%20stop%20other%20processes%20and%20trigger%20%20%20%20-%20**And**%20the%20observer%20process%20should%20be%20seen%20to%20sever%20the%20lifeline%20%20%20%20-%20**And**%20the%20payload%20process%20should%20be%20seen%20to%20notice%20the%20lifeline%20has%20failed-%20**Then**%20when%20the%20payload%20process%20is%20noted%20to%20trigger%20%20-%20**Then**%20the%20execution%20phase%20of%20the%20payload%20is%20noted%20to%20have%20succeeded%20%20%20%20-%20**And**%20the%20alert%20phase%20of%20the%20payload%20is%20noted%20to%20have%20succeeded%20%20%20%20-%20**And**%20the%20verification%20phase%20of%20the%20payload%20is%20noted%20to%20have%20suceeded%20%20%20%20-%20**And**%20the%20payload%20function%20is%20noted%20to%20have%20exited%20successfully%20%20%20%20-%20**And**%20the%20payload%20process%20is%20noted%20to%20have%20attempted%20to%20sever%20the%20lifeline%20%20%20%20-%20**And**%20the%20payload%20process%20appears%20to%20exit%20cleanly%20%20-%20**Then**%20the%20recipient%20e-mail&#39;s%20inbox%20should%20be%20manually%20observed%20%20%20%20-%20**And**%20the%20receipt%20of%20the%20e-mail%20should%20be%20confirmed%20%20%20%20-%20**And**%20the%20contents%20of%20the%20e-mail%20should%20be%20verified%20to%20be%20the%20same%20as%20provided%20in%20`MSG_CONTENTS`%20%20%20%20-%20**And**%20it%20should%20be%20verified%20that%20the%20e-mail%20has%20**NOT**%20been%20signed%20with%20a%20PGP%20key%20%20%20%20-%20**And**%20it%20should%20be%20verified%20that%20the%20e-mail%20**HAS**%20been%20encrypted%20with%20a%20PGP%20key-%20**Then**%20the%20directory%20containing%20the%20`.env`%20file%20should%20be%20manually%20observed%20%20-%20**And**%20it%20should%20be%20confirmed%20that%20the%20`.env`%20file%20has%20been%20deleted-%20**Then**%20the%20directory%20containing%20the%20`PGP_PUBKEY`%20file%20should%20be%20manually%20observed%20%20-%20**And**%20it%20should%20be%20confirmed%20that%20the%20`PGP_PUBKEY`%20file%20has%20been%20deleted|%20Step%20|%20Instruction%20|%20Expected%20Outcome%20|%20Test%20Data%20|%20Actual%20Outcome%20|%20Result%20||%20:--:%20|%20:----------%20|%20:---------------%20|%20:--------%20|%20:-------------%20|%20:----:%20||%201%20|%20Confirm%20that%20a%20valid%20`.env`%20file%20with%20all%20required%20environment%20variables%20exists%20in%20the%20test%20directory,%20including%20the%20`PGP_PUBKEY`%20environment%20variable.%20|%20A%20`.env`%20file%20is%20present%20in%20the%20test%20directory%20that%20has%20all%20required%20variables.%20|%20N/A%20|%20As%20expected.%20|%20PASS%20||%202%20|%20Confirm%20that%20the%20path%20provided%20by%20`PGP_PUBKEY`%20%20is%20a%20valid%20PGP%20public%20key%20via%20`gpg%20--show-keys%20--with-fingerprint"/>PGP_PUBKEY`. | The command returns information indiciating that this is a valid PGP public key capable of encrypting messages for the specified recipient. | `gpg --show-keys --with-fingerprint Bob.public.asc` | `pub   ed25519 2024-03-31 [SC] [expires: 2026-03-31]      6EDC CCEE EE03 86D8 F0F3  7F86 17A1 9093 7B15 9DA3 uid                      Bob Lastname <bob@example.com> sub   cv25519 2024-03-31 [E] [expires: 2026-03-31]` | PASS | 
| 3 | Start the DMS launcher program with valid arguments, ensuring that the `--alert` payload function argument flag has been provided and with observer function arguments that are likely to cause a trigger event in the current test environment. | The DMS launcher starts without issue. | `.\dms-launcher.py -p ".\payload.py localhost 5455 .\secrets\ --heartbeat-grace-period 5 --heartbeat-max-retries 5 --heartbeat-timeout 5 --handshake-timeout 5 --alert --log-level INFO" -o ".\observer.py localhost 5455 --heartbeat-grace-period 5 --heartbeat-max-retries 5 --heartbeat-timeout 5 --handshake-timeout 5 --reject-noframe --log-level INFO" --timeout 5 --retries 0 --env .\Scripts\ --log-level INFO` | As expected. | PASS |
...
| 2 | Confirm that the path provided by `PGP_PRIVKEY` is a valid PGP private key via `gpg --show-keys --with-fingerprint <img src="https://latex.codecogs.com/gif.latex?PGP_PRIVKEY`.%20|%20The%20command%20returns%20information%20indicating%20that%20this%20is%20a%20valid%20PGP%20private%20key%20capable%20of%20signing%20messages.%20|%20`gpg%20--show-keys%20--with-fingerprint%20dms.priv.asc`%20|%20`sec%20%20%20ed25519%202024-03-31%20[SC]%20[expires:%202026-03-31]%20%20%20%20%20%205F53%201337%20BAA9%2085A3%20F307%20%20938B%202C2D%209793%2062FC%20424F%20uid%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20Dead%20Man&#39;s%20Switch%20&lt;dms@example.com&gt;%20ssb%20%20%20cv25519%202024-03-31%20[E]%20[expires:%202026-03-31]`%20|%20PASS%20||%203%20|%20Confirm%20that%20the%20path%20provided%20by%20`PGP_PUBKEY`%20is%20a%20valid%20PGP%20public%20key%20via%20`gpg%20--show-keys%20--with-fingerprint"/>PGP_PUBKEY`. | The command returns information indiciating that this is a valid PGP public key capable of encrypting messages for the specified recipient. | `gpg --show-keys --with-fingerprint Bob.public.asc` | `pub   ed25519 2024-03-31 [SC] [expires: 2026-03-31]      6EDC CCEE EE03 86D8 F0F3  7F86 17A1 9093 7B15 9DA3 uid                      Bob Lastname <bob@example.com> sub   cv25519 2024-03-31 [E] [expires: 2026-03-31]` | PASS |
| 4 | Start the DMS launcher program with valid arguments, ensuring that the `--alert` payload function argument flag has been provided and with observer function arguments that are likely to cause a trigger event in the current test environment. | The DMS launcher starts without issue. | `.\dms-launcher.py -p ".\payload.py localhost 5455 .\secrets\ --heartbeat-grace-period 5 --heartbeat-max-retries 5 --heartbeat-timeout 5 --handshake-timeout 5 --alert --log-level INFO" -o ".\observer.py localhost 5455 --heartbeat-grace-period 5 --heartbeat-max-retries 5 --heartbeat-timeout 5 --handshake-timeout 5 --reject-noframe --log-level INFO" --timeout 5 --retries 0 --env .\Scripts\ --log-level INFO` | As expected. | PASS |
